...
public class UserObedient extends UserUninformed {
    ...
    public UserDecision decideAfterFailedRental() {
        ...
        if (printHints) {
            System.out.format("[UserInfo] User: %3d Failed take. last probability was:  %9.8f; has walked %f meters of %d maximum.%n",
...
//        } else {
    ...
    public UserDecision decideAfterFailedReturn() {
        if (printHints) {
            System.out.format("[UserInfo] User: %3d Failed return. last probability was:  %9.8f %n", this.getId(), lastexpetedProbability);
        }
    ...
    protected Station determineStationToRentBike() {
        ...
        if (printHints) {
            System.out.format("[UserInfo] User: %3d asks for renting recommendation; has walked %f meters of %d maximum.%n",
    ...
    protected Station determineStationToReturnBike() {
        Station destination = null;
        if (printHints) {
            System.out.format("[UserInfo] User: %3d asks for return recommendation; has walked %f meters of %d maximum.%n",
                    this.getId(), getMemory().getWalkedToTakeBikeDistance(), parameters.maxDistanceToRentBike);
        }
