...
public class RecommendationSystemDemandProbabilityCostGlobalPrediction extends RecommendationSystemDemandProbabilityBased {
    ...
    public class RecommendationParameters {
        ...
        private double MaxCostValue = 2000000;
        private double maxStationsToReccomend = 30;
        private double unsucesscostRent = 3000;//3000
        private double unsucesscostReturn = 2000;//2000
        private double factordemand = 0.01; 
    ...
    private double calculateCostsRentAtStation(StationUtilityData sd,
        ...
        double usercosttake = calculateWayCostRentHeuristic(way,sd, sd.getProbabilityTake(), 1, sd.walkdist, lookedlist, allstats, true);

        //analyze global costs
        List<StationUtilityData> auxway = new LinkedList<StationUtilityData>();
        double margprob = 1;
        ...
        for (StationUtilityData wp : way) {
            ...
            double costtake = calculateWayCostRentHeuristic(auxway, wp, wp.getProbabilityTake(), 1, 0, newlookedlist, allstats, false);
            newlookedlist=new ArrayList<>(lookedlist);
            double costtakeafter = calculateWayCostRentHeuristic(auxway, wp, wp.getProbabilityTakeAfter(), 1, 0, newlookedlist, allstats, false);
            ...
            newlookedlist=new ArrayList<>(lookedlist);
            double costreturnhip = calculateWayCostReturnHeuristic(auxway,wp, wp.getProbabilityReturn(), 1, 0, hipodestination, newlookedlist, allstats, false);
            newlookedlist=new ArrayList<>(lookedlist);
            double costreturnafterhip = calculateWayCostReturnHeuristic(auxway,wp, wp.getProbabilityReturnAfter(), 1, 0, hipodestination, newlookedlist, allstats, false);
            ...
            double futtakedemand = infrastructureManager.getFutureBikeDemand(wp.getStation(), timeoffset);
            double futreturndemand = infrastructureManager.getFutureSlotDemand(wp.getStation(), timeoffset);
            ...
            difcosttake = difcosttake *  futtakedemand * this.parameters.factordemand;
            difcostreturn = difcostreturn* futreturndemand * this.parameters.factordemand;
            ...
            if (newmargprob <= this.parameters.minimumMarginProbability) {
                acctakecost+= (margprob - this.parameters.minimumMarginProbability)*difcosttake;
                accreturncost+= (margprob - this.parameters.minimumMarginProbability)*difcostreturn;
            }
            else {
        ...
        sd.setCost(usercosttake).setTakecostdiff(acctakecost).setReturncostdiff(accreturncost);
        return globalcost;
    ...
    private double calculateCostsReturnAtStation(StationUtilityData sd, GeoPoint destination,
        ...
        List<StationUtilityData> lookedlist = new ArrayList<>();
        List<StationUtilityData> way = new LinkedList<StationUtilityData>();
        double usercostreturn = calculateWayCostReturnHeuristic(way, sd, sd.getProbabilityReturn(), 1, sd.bikedist, destination, lookedlist, allstats, true);
        ...
        sd.setCost(usercostreturn).setTakecostdiff(acctakecost).setReturncostdiff(accreturncost);
        return globalcost;
    }
