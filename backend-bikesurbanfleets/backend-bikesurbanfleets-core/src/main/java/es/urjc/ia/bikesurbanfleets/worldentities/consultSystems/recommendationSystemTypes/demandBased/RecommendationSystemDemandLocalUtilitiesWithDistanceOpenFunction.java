...
public class RecommendationSystemDemandLocalUtilitiesWithDistanceOpenFunction extends RecommendationSystem {
    ...
    private double getIdealBikes(Station s) {
        ...
        DemandManager.DemandResult takedem = demandManager.getTakeDemandStation(s.getId(), DemandManager.Month.toDemandMangerMonth(current.getMonth()),
    ...
    private double getIdealSlots(Station s) {
        LocalDateTime current = SimulationDateTime.getCurrentSimulationDateTime();
        DemandManager.DemandResult retdem = demandManager.getReturnDemandStation(s.getId(), DemandManager.Month.toDemandMangerMonth(current.getMonth()),
                DemandManager.Day.toDemandMangerDay(current.getDayOfWeek()), current.getHour());
        if (retdem.hasDemand()) {
