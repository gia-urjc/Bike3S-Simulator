...
public class History {
     ...
     *
     * @param globalInfo contains the global parameters of the simulation
     ...
     */
    public static void init(GlobalInfo globalInfo, List<Entity> initialEntities) throws IOException {
        //initialize values
        activeEntities = new HistoricEntityCollection();
        serializedEvents = new TreeMap<Integer, List<EventEntry>>();
        outputPath = Paths.get(globalInfo.getHistoryOutputPath());
        File outputDirectory = new File(outputPath.toString());
        if (outputDirectory.exists() && outputDirectory.isDirectory()) {
            outputDirectory.mkdirs();
        }
        TIMEENTRIES_PER_FILE = globalInfo.TIMEENTRIES_PER_HISTORYFILE;
        //write the global information in a file
        FinalGlobalValues finalGlobalValues = new FinalGlobalValues(globalInfo);
    ...
    private static class FinalGlobalValues {
        ...
        public FinalGlobalValues(GlobalInfo globalinfo) {
            this.totalTimeSimulation = globalinfo.getTotalSimulationTime();
            this.boundingBox = globalinfo.getBoundingBox();
        }
    }
}
