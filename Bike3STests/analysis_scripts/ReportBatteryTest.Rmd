...
params:
  path: /Users/ibernabe/IntellijProjects/Bike3STests/paperAT2018/allbikes/test/09_11_2018_13_12_15/analisis
  #path: /Users/ibernabe/IntellijProjects/Bike3STests/paperAT2018/allbikes/test/31_10_2018_12_06_41/analisis
---
...
for (k in 1:length(listcsv)){
  ...
  listcsv[k] <- sapply(listcsv[k],function(x) {x <- gsub("/users.csv","",x)
                                                             x <-  gsub(params$path,"",x)
                                                            x <-   gsub("/","",x)
                                                                })
  
  #Creo una columna por cada iteración
  value <-format( mean (timeToStation)/60, digits=2, nsmall=2)
  timeToStationAux <- t(c(listcsv[k],value))
  
  value <-format( mean (cyclingTime)/60, digits=2, nsmall=2)
  cyclingTimeAux <- t(c(listcsv[k],value))
  
  value <-format( mean (timeToDestinationPlace)/60, digits=2, nsmall=2)
...
for (k in 1:length(listcsv)){
  ...
  stations  <- read.csv(listcsv[k],
   ...
   #Modifico los textos en el dataframe
  listcsv[k] <- sapply(listcsv[k],function(x) {x <- gsub("/stationBalanceQuality.csv","",x)
                                                             x <-  gsub(params$path,"",x)
                                                            x <-   gsub("/","",x)
                                                                })
...
for (k in 1:length(listcsv)){
  ...
  valores  <- read.csv(listcsv[k],
   ...
   #Modifico los textos en el dataframe
  listcsv[k] <- sapply(listcsv[k],function(x) {x <- gsub("/global_values.csv","",x)
                                                             x <-  gsub(params$path,"",x)
                                                            x <-   gsub("/","",x)
                                                                })
  path <- listcsv[k]
