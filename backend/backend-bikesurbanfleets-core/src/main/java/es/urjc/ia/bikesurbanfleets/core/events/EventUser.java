...
import es.urjc.ia.bikesurbanfleets.core.entities.users.UserMemory;
...
public abstract class EventUser implements Event {
    ...
    public List<Event> manageBikeReservation(Station destination) throws Exception{
        ...
        if (bike != null) {  // user has been able to reserve a bike  
            Reservation reservation = new Reservation(instant, Reservation.ReservationType.BIKE, user, destination, bike);
            ...
            Reservation reservation = new Reservation(instant, Reservation.ReservationType.BIKE, user, destination);
    ...
    public List<Event> manageSlotReservation(Station destination) throws Exception{
        ...
        if (user.reservesSlot(destination)) {  // User has been able to reserve
            Reservation reservation = new Reservation(instant, Reservation.ReservationType.SLOT, user, destination, user.getBike());
               if (Reservation.VALID_TIME < arrivalTime) {
                   user.setRoute(user.reachedRouteUntilTimeOut());
                   ...
                   Reservation reservation = new Reservation(instant, Reservation.ReservationType.SLOT, user, destination);
                   user.addReservation(reservation);
