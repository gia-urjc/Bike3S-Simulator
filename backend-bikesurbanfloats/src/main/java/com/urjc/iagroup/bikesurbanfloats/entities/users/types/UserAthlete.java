...
public class UserAthlete extends User {
	...
	/**
	 * It is the minimum distance that the user wants to travel since he rents a bike
	 * until he returns it.
	 */
	private int minDistanceToTravel;
	
	/**
	 * It is the distance that the user travels by bike.
	 */
	private int traveledDistanceByBike;
    ...
    public Station determineStationToReturnBike(int instant) {
        ...
        while (!stations.isEmpty()) {
        	...
        	for(GeoRoute route: routes) {
        		if(route.getTotalDistance() >= minDistanceToTravel - traveledDistanceByBike)
        			return destination;
        	}
    ...
    public GeoRoute determineRoute(List<GeoRoute> routes) throws GeoRouteException {
        ...
        GeoRoute selectedRoute = routes.get(0);
        if(hasBike()) {
        	traveledDistanceByBike += selectedRoute.getTotalDistance();
        }
